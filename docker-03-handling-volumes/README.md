...
- Launch a Docker machine on Amazon Linux 2023 AMI with security group allowing SSH connections using the [Cloudformation Template for Docker Machine Installation](../docker-01-installing-on-ec2-linux2/docker-installation-template.yml).
...
- Run a `alpine` container with interactive shell open, name the container as `ogulcan`, attach the volume `my-vol` to `/test` mount point in the container, and add command to run alpine shell. Here, explain `--volume` and `v` flags.

```bash
docker run -it --name ogulcan -v my-vol:/test alpine ash
```

- List files/folder in `ogulcan` container, show mounting point `/test`, and explain the mounted volume `my-vol`.

```bash
ls
```

- Create a file in `ogulcan` container under `/test` folder.

```bash
cd test && echo "This file is created in the container ogulcan" > i-will-persist.txt
```
...
- Exit the `ogulcan` container and return to ec2-user bash shell.
...
- Remove the `ogulcan` container.

```bash
docker rm ogulcan
```

- Show the list of all containers, and the `ogulcan` container is gone.
...
- Run a `alpine` container with interactive shell open, name the container as `ogulcan2`, attach the volume `my-vol` to `/test2` mount point in the container, and add command to run alpine shell.

```bash
docker volume ls
docker run -it --name ogulcan2 -v my-vol:/test2 alpine ash
```
...
- Create an another file in `ogulcan2` container under `/test2` folder.

```bash
cd test2 && echo "This is a file of the container ogulcan2" > loadmore.txt
```
...
- Exit the `ogulcan2` container and return to ec2-user bash shell.

```bash
exit
```

- Run a `ubuntu` container with interactive shell open, name the container as `ogulcan3`, attach the volume `my-vol` to `/test3` mount point in the container, and add command to run bash shell.

```bash
docker run -it --name ogulcan3 -v my-vol:/test3 ubuntu bash
```
...
- Create an another file in `ogulcan3` container under `/test3` folder.

```bash
cd test3 && touch file-from-3rd.txt && ls
```

- Exit the `ogulcan3` container and return to ec2-user bash shell.

```bash
exit
```

- Run an another `ubuntu` container with interactive shell open, name the container as `ogulcan4`, attach the volume `my-vol` as read-only to `/test4` mount point in the container, and add command to run bash shell.

```bash
docker run -it --name ogulcan4 -v my-vol:/test4:ro ubuntu bash
```
...
- Exit the `ogulcan4` container and return to ec2-user bash shell.
...
- Delete `ogulcan2`, `ogulcan3` and `ogulcan4` containers.

```bash
docker rm ogulcan2 ogulcan3 ogulcan4
```
...
- Run the `ogulcancompany/hello-ogulcan` container with interactive shell open, name the container as `ogulcan`, and show the inside of `hello-ogulcan` directory.

```bash
docker run -it --name ogulcan ogulcancompany/hello-ogulcan sh
/ # ls
...
- Run the `ogulcancompanys/hello-ogulcan` container with interactive shell open, name the container as `try1`, attach the volume `full-vol` to `/test` mount point in the container, and show that `/test` directory is created and files inside volume are copied to this directory.
...
- Run the `ogulcancompany/hello-ogulcan` container with interactive shell open, name the container as `try2`, attach the volume `empty-vol` to `/hello-ogulcan` mount point in the container.
...
- Run the `ogulcancompany/hello-ogulcan` container with interactive shell open, name the container as `try3`, attach the volume `full-vol` to `/hello-ogulcan` mount point in the container, and show that we just see the files inside volume regardless of the target directory is full or empty.
...
docker run -it --name ogulcan-test -v volume1:/hello-ogulcan -v volume2:/app -v volume3:/logs ogulcancompany/hello-ogulcan sh
exit
...
echo "<h1>Welcome to ogulcancompany</h1>" > index.html
```
...
<h1>Welcome to ogulcancompany</h1>
```
...
docker attach 5 \_\_> attantion !! when you exit then the container prosess finished so that container also stopped
