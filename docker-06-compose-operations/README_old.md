...
- Important Note: ```docker-compose```requres ```buildx``` version 0.17 and above. Check ```buildx``` version

```bash
docker buildx version
```

- If ```buildx``` version is not above 0.17 then update ```buildx```.

```bash
mkdir ~/.docker/cli-plugins
wget -O ~/.docker/cli-plugins/docker-buildx https://github.com/docker/buildx/releases/download/v0.30.1/buildx-v0.30.1.linux-amd64
