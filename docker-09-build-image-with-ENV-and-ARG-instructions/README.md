# Hands-on Docker-09: Build images with ENV and ARG instructions
...
- Build images with `ENV` form `ARG` instruction.
...
```bash
docker build -t ogulcan/myweb:nginx .
```

- Run this image and check the result in your browser.

```bash
docker run --name myweb -dp 80:80 ogulcan/myweb:nginx
```
...
```bash
docker build -t ogulcan/myweb:arg .
```

- Run this image and see in the browser that the background is pink.

```bash
docker run --name myweb-arg -dp 80:80 ogulcan/myweb:arg
```

- Build an image from this Dockerfile with `build-arg` variable.

```bash
docker build -t ogulcan/myweb:arg-gray --build-arg COLOR=gray .
```

- Run this image and see in the browser that the background is gray.

```bash
docker run --name myweb-arg-gray -dp 81:80 ogulcan/myweb:arg-gray
```
